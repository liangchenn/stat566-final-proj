...
balance_long[, covariate := factor(covariate, levels = balance[order(raw), covariate])] |> invisible()
...
ggplot(rhc) +
    aes(x = ps, fill = factor(get(treat_var)))+
...
```{r}
ggplot(rhc) +
    aes(x = ps, fill = factor(get(treat_var)), weight = ate_weight)+
    geom_density(alpha = 0.45) +
    labs(
        x = "Estimated propensity score",
        y = "Density",
...
```{r}
ggplot(rhc) +
    aes(x = ps, fill = factor(get(treat_var)), weight = att_weight)+
    geom_density(alpha = 0.45) +
    labs(
        x = "Estimated propensity score",
        y = "Density",

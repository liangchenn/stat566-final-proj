...
bootstrap_B <- as.integer(Sys.getenv("BOOTSTRAP_B", "100"))
bootstrap_seed <- 2026
conf_level <- 0.95
...
estimate_naive_log_or_se <- function(data, outcome_var, treat_var, correction = 0.5) {
    A <- data[[treat_var]]
    Y <- data[[outcome_var]]
...
estimate_ate_aipw_or <- function(data, outcome_var, treat_var, covariates, trim = c(0.02, 0.98)) {
    outcome_fit <- glm(
...
estimate_att_aipw_or <- function(data, outcome_var, treat_var, covariates, trim = c(0.02, 0.98)) {
...
estimate_ps_matching_log_or_se <- function(data,
                                           outcome_var,
                                           treat_var,
                                           covariates,
                                           estimand = c("ATE", "ATT"),
...
estimate_all_methods <- function(data, outcome_var, treat_var, covariates) {
    data.table(
        ...
        Estimate = c(
            ...
            estimate_regression_ate_or(data, outcome_var, treat_var, covariates),
            estimate_ate_ipw_or(data, outcome_var, treat_var, covariates),
            estimate_ps_matching_or(data, outcome_var, treat_var, covariates, estimand = "ATE"),
            estimate_ate_aipw_or(data, outcome_var, treat_var, covariates),
            estimate_regression_att_or(data, outcome_var, treat_var, covariates),
            estimate_att_ipw_or(data, outcome_var, treat_var, covariates),
            estimate_ps_matching_or(data, outcome_var, treat_var, covariates, estimand = "ATT"),
            estimate_att_aipw_or(data, outcome_var, treat_var, covariates)
        )
    )
}

estimate_bootstrap_methods <- function(data, outcome_var, treat_var, covariates) {
    data.table(
...
comparison[Method == "Naive comparison", log_or_se := estimate_naive_log_or_se(
    rhc,
    outcome_var,
    treat_var
)]
comparison[Estimand == "ATE" & Method == "PS matching", log_or_se :=
    estimate_ps_matching_log_or_se(
        rhc,
        outcome_var,
        treat_var,
        covariates,
...
comparison[Estimand == "ATT" & Method == "PS matching", log_or_se :=
    estimate_ps_matching_log_or_se(
        rhc,
        outcome_var,
        treat_var,
